...
const dbInit = {
	...
	async v1_6DB(c) {
		...
		const noticeContent = 'This project is for learning and educational exchange only. Do not use it for illegal activities.<br>Please comply with local laws. The author assumes no legal liability.'
	...
	async v1_5DB(c) {
		...
		try {
			...
			console.warn(`skip add column: ${e.message}`);
		}
	},
    ...
    `).run();
        ...
        (34,'View invite codes', 'reg-key:query', 33, 2, 0),
    ...
    `).run();
        ...
        (6, 'User management', NULL, 0, 1, 3),
        (7, 'View users', 'user:query', 6, 2, 0),
        (8, 'Change password', 'user:set-pwd', 6, 2, 2),
        (9, 'Change status', 'user:set-status', 6, 2, 3),
        (10, 'Change permissions', 'user:set-type', 6, 2, 4),
        (11, 'Delete user', 'user:delete', 6, 2, 7),
        (12, 'Star users', 'user:star', 6, 2, 5),
        (13, 'Access control', '', 0, 1, 5),
        (14, 'View roles', 'role:query', 13, 2, 0),
        ...
        (22, 'View mailboxes', 'account:query', 21, 2, 0),
        (23, 'Add mailbox', 'account:add', 21, 2, 1),
        (24, 'Delete mailbox', 'account:delete', 21, 2, 2),
        (25, 'Add user', 'user:add', 6, 2, 1),
        (26, 'Reset send quota', 'user:reset-send', 6, 2, 6),
        (27, 'Mail list', '', 0, 1, 4),
        (28, 'View all mail', 'all-email:query', 27, 2, 0),
        (29, 'Delete all mail', 'all-email:delete', 27, 2, 0),
    ...
    `).run();
        ...
        INSERT INTO role (
          ...
          1, 'User', NULL, '0000-00-00 00:00:00', 0, 'Basic usage permissions only', 0, 1, NULL, 'ban', 10
        )
      `).run();
		}
