...
const emailService = {
	...
	async list(c, params, userId) {
		...
		if (isNaN(timeSort)) timeSort = 0;

		if (isNaN(accountId)) {
			allReceive = 1;
		} else if (isNaN(allReceive)) {
	...
	async allList(c, params) {
		...
		// Guard missing size - bare /allEmail/list used to pull unbounded rows
		// then blow D1 on attachment IN (...) binds.
