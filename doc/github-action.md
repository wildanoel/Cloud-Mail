...
> Note: this package ships without a committed `.github/workflows/*.yml` by default so a `repo`-only PAT can push. Add your own workflow later if needed (requires `workflow` scope).
